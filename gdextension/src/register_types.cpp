#include "register_types.h"

#include "game_engine.h"
#include "game_map.h"
#include "game_player.h"
#include "game_unit.h"
#include "game_actions.h"
#include "game_setup.h"

#include <gdextension_interface.h>
#include <godot_cpp/core/defs.hpp>
#include <godot_cpp/core/class_db.hpp>
#include <godot_cpp/godot.hpp>

using namespace godot;

void initialize_maxtreme_module(ModuleInitializationLevel p_level) {
    if (p_level != MODULE_INITIALIZATION_LEVEL_SCENE) {
        return;
    }

    ClassDB::register_class<GameEngine>();
    ClassDB::register_class<GameMap>();
    ClassDB::register_class<GamePlayer>();
    ClassDB::register_class<GameUnit>();
    ClassDB::register_class<GameActions>();
    ClassDB::register_class<GameSetup>();
}

void uninitialize_maxtreme_module(ModuleInitializationLevel p_level) {
    if (p_level != MODULE_INITIALIZATION_LEVEL_SCENE) {
        return;
    }
}

extern "C" {
GDExtensionBool GDE_EXPORT maxtreme_library_init(
    GDExtensionInterfaceGetProcAddress p_get_proc_address,
    const GDExtensionClassLibraryPtr p_library,
    GDExtensionInitialization *r_initialization
) {
    godot::GDExtensionBinding::InitObject init_obj(p_get_proc_address, p_library, r_initialization);

    init_obj.register_initializer(initialize_maxtreme_module);
    init_obj.register_terminator(uninitialize_maxtreme_module);
    init_obj.set_minimum_library_initialization_level(MODULE_INITIALIZATION_LEVEL_SCENE);

    return init_obj.init();
}
}
